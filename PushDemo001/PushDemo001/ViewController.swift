...
class ViewController: UIViewController {
    ...
    func createUI()  {
        ...
        let btnArray:[String] = ["iOS10以下本地推送测试","iOS10本地推送测试","iOS10自定义交互的通知","iOS10附件通知"]
        let btnBackGroundColorArray:[UIColor] = [UIColor.red,UIColor.green,UIColor.cyan,UIColor.blue]
        
        for i in 0..<btnArray.count {
            // 创建测试的 button
            let testBtn = UIButton.init(type: UIButtonType.custom)
            testBtn.frame = CGRect(x: 20, y: 50+i*60, width: 200, height: 40)
